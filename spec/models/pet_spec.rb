  ...
  describe 'instance methods' do
    ...
    describe '.actions_required' do
      it 'returns the applications where action is required for pet'  do
        ...
        application_pet_1_scott = @pet_1.application_pets.find_by(application_id: scott.id)
        application_pet_2_scott = @pet_2.application_pets.find_by(application_id: scott.id)

        application_pet_1_bob = @pet_1.application_pets.find_by(application_id: bob.id)
        application_pet_2_bob = @pet_2.application_pets.find_by(application_id: bob.id)
        application_pet_1_bob.update(status: 'Approved')

        application_pet_3_sierra = @pet_3.application_pets.find_by(application_id: sierra.id)
        application_pet_3_sierra.update(status: 'Approved')

        application_pet_1_laura = @pet_1.application_pets.find_by(application_id: laura.id)
        application_pet_2_laura = @pet_2.application_pets.find_by(application_id: laura.id)
        application_pet_1_laura.update(status: 'Rejected')
        application_pet_2_laura.update(status: 'Approved')
