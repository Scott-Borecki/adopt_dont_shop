...
RSpec.describe 'the veterinary offices veterinarians index' do
  ...
  it 'lists all the veterinarians associated with the office, with their '\
     'attributes' do
  ...
  it 'displays a link to create a new veterinarian' do
    ...
    expect(page).to have_current_path("/veterinary_offices/#{@vet_office_1.id}"\
                                      "/veterinarians/new")
  end
  ...
  it 'displays a form for a number value' do
    ...
    expect(page).to have_content("Only display veterinarians with a review "\
                                 "rating of at least...")
